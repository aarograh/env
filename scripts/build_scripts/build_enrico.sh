...
####### This if for a "standard" enrico build using cmake only
    ...
    cmake \
      ...
      -D CMAKE_INCLUDE_PATH="~/code_installations/scale_master/trilinos_install/include;~/code_installations/scale_master/install/include" \
      -D CMAKE_LIBRARY_PATH="~/code_installations/scale_master/trilinos_install/lib;~/code_installations/scale_master/install/lib" \
      -D CMAKE_PREFIX_PATH=~/code_installations/scale_master/install \
      -D Trilinos_DIR=~/code_installations/scale_master/trilinos_install \
      -D SCALE_DIR=~/code_installations/scale_master/install \
      $@

######## Below is for my attempt at a Futility/TriBITS build
